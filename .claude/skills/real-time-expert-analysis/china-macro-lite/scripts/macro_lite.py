...
def main():
    ...
    if args.category:
        indicators = [ind for ind in indicators if ind["category"] == args.category]
        if not indicators:
            valid_cats = sorted(set(ind["category"] for ind in INDICATORS))
            print(
                "\n[错误] 未知类别: '{}'".format(args.category),
                file=sys.stderr,
            )
            print(
                "可用类别: {}".format(", ".join(valid_cats)),
                file=sys.stderr,
            )
            sys.exit(2)
        print("📋 筛选类别: {}".format(args.category))
