...
### Mode A: 独立运行
用户直接调用 `/earnings-report-extractor`。Skill 自行下载 PDF、提取章节，直接输出给用户。

### Mode B: 协同运行（工作流调用）
工作流 Lead Agent 调用，指定输出目录。

**下载阶段：**
```bash
python .claude/skills/components/earnings-report-extractor/scripts/report_extractor.py \
  --code {stock_code} --year {year} --report-type {type} \
...
python .claude/skills/components/earnings-report-extractor/scripts/report_extractor.py \
  ...
  --sections mda,risk,chairman_letter,business,audit
```

**输出契约：**
- PDF 文件: `data/{stock_code}/earnings-pdf/{code}_*.pdf`
- 提取章节: `data/{stock_code}/earnings-extracted/{code}_第X节_*.md`
- 结构索引: `data/{stock_code}/earnings-extracted/{code}_structure.json`

**质量门：**
- 下载: PDF 文件存在且 > 500KB（年报）
- 提取: MD&A 文件存在且字符数 > 1000
