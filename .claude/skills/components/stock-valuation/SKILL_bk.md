...
### Mode A: 独立运行
用户直接调用 `/stock-valuation`。Skill 自行获取数据并执行八模型估值，直接输出给用户。

### Mode B: 协同运行（工作流调用）
工作流调用，从缓存读取数据，避免重复 API 请求。
...
**输入契约:** `specs/contracts/financial-statements.json`（数据收集输入）
**中间契约:** `specs/contracts/valuation-input.json`（预处理输出）
**输出:** `data/{stock_code}/valuation-reports/{code}_{name}_估值分析报告.md`
