...
class ReportBuilder:
    ...
    def _get_models_summary(self):
        ...
        def _parse_model(key, val):
            ...
            weight = 0
            if self.integrated:
                # 尝试从 weighted_models 列表获取
                weighted_models = self.integrated.get("weighted_models", [])
                if isinstance(weighted_models, list):
                    for wm in weighted_models:
                        if wm.get("model_name") == model_name or wm.get("model") == model_name:
                            weight = wm.get("final_weight") or wm.get("weight") or 0
                            break
                # 尝试从 model_weights dict 获取
                if weight == 0:
                    model_weights = self.integrated.get("model_weights", {})
                    if isinstance(model_weights, dict) and model_name in model_weights:
                        mw = model_weights[model_name]
                        if isinstance(mw, dict):
                            weight = mw.get("weight") or mw.get("final_weight") or 0
                        else:
                            weight = mw or 0

            # 回退到 val 本身
            if weight == 0:
                weight = (val.get("final_weight")
                          or val.get("weight") or 0)
