...
def _extract_name_from_records(*record_lists):
    """从财报 raw_records 中提取公司名称（SECURITY_NAME_ABBR 字段）。"""
...
class RealTimeQuote:
    ...
    def fetch(self, code):
        ...
        result = {
            "current_price": None, "change_pct": None, "volume": None,
        ...
        # Sina fallback: 当 push2 未返回 current_price 时尝试新浪财经
        if result["current_price"] is None:
            print("  [INFO] push2 未返回价格，尝试新浪财经 fallback...")
            sina = _fetch_sina_quote(code)
            if sina and sina.get("current_price") is not None:
                result["current_price"] = sina["current_price"]
                if sina.get("name"):
                    result["name"] = sina["name"]
                print(f"  [INFO] 新浪 fallback 成功: price={sina['current_price']}, name={sina.get('name', '')}")
            else:
                print("  [警告] 新浪 fallback 也未返回数据")
...
class ValuationDataCollector:
    ...
    def collect_all(self):
        ...
        result["income_data"] = {
            ...
            "raw_records": fin_data["income"].get("records", []),
        ...
        balance_built["raw_records"] = fin_data["balance"].get("records", [])
        ...
        result["cashflow_data"] = {
            ...
            "raw_records": fin_data["cashflow"].get("records", []),
        ...
        # 回退计算：确保关键字段有值
        balance_latest = result.get("balance_data", {}).get("latest", {})

        # total_shares 回退链
        # 1) 从 rt（push2 行情）获取
        # 2) 从 total_market_cap / current_price 推算
        # 3) 从 net_profit / eps 推算（财报数据）
        price_val = rt.get("current_price")
        mcap_val = rt.get("total_market_cap")
        if not rt.get("total_shares") and mcap_val and price_val and price_val > 0:
            rt["total_shares"] = round(mcap_val / price_val, 4)
            print("  [回退] total_shares 从 market_cap/price 推算")
        if not rt.get("total_shares"):
            # 从 net_profit_latest / eps_latest 推算
            eps_list = income_series.get("eps", [])
            np_list = income_series.get("net_profit", [])
            sorted_eps = sorted([e for e in eps_list if e.get("value") is not None and e["value"] != 0],
                                key=lambda x: x.get("year", 0), reverse=True)
            sorted_np = sorted([e for e in np_list if e.get("value") is not None],
                               key=lambda x: x.get("year", 0), reverse=True)
            if sorted_eps and sorted_np:
                eps_latest = sorted_eps[0]["value"]
                np_latest_yi = sorted_np[0]["value"]  # 亿元
                if eps_latest and eps_latest != 0:
                    # net_profit 已经是亿元，total_shares 也转为亿股
                    rt["total_shares"] = round(np_latest_yi / eps_latest, 4)
                    print(f"  [回退] total_shares 从 net_profit/eps 推算: {rt['total_shares']} 亿股")
        # 同步 total_shares 到 balance_data.latest
        if rt.get("total_shares") and balance_latest:
            if not balance_latest.get("total_shares"):
                balance_latest["total_shares"] = rt["total_shares"]

        # eps_ttm 回退链
        # 1) 从 income_data eps 序列获取最近年报值
        # 2) 从 net_profit_latest / total_shares 推算
        if not rt.get("eps_ttm"):
            eps_list = income_series.get("eps", [])
            sorted_eps = sorted([e for e in eps_list if e.get("value") is not None],
                                key=lambda x: x.get("year", 0), reverse=True)
            if sorted_eps:
                rt["eps_ttm"] = sorted_eps[0]["value"]
                print(f"  [回退] eps_ttm 从 eps 序列获取: {rt['eps_ttm']}")
        if not rt.get("eps_ttm"):
            # 从 net_profit / total_shares 推算
            np_list = income_series.get("net_profit", [])
            sorted_np = sorted([e for e in np_list if e.get("value") is not None],
                               key=lambda x: x.get("year", 0), reverse=True)
            ts_val = rt.get("total_shares")
            if sorted_np and ts_val and ts_val > 0:
                # net_profit 是亿元, total_shares 是亿股 => eps 单位正确
                rt["eps_ttm"] = round(sorted_np[0]["value"] / ts_val, 4)
                print(f"  [回退] eps_ttm 从 net_profit/total_shares 推算: {rt['eps_ttm']}")

        # bvps 回退：从 total_equity / total_shares 推算
        if not rt.get("bvps"):
            if balance_latest.get("bvps"):
                rt["bvps"] = balance_latest["bvps"]
                print(f"  [回退] bvps 从 balance_data.latest 获取: {rt['bvps']}")
        if not rt.get("bvps"):
            total_eq = balance_latest.get("total_equity")  # 亿元
            ts_val = balance_latest.get("total_shares") or rt.get("total_shares")  # 亿股
            if total_eq and ts_val and ts_val > 0:
                rt["bvps"] = round(total_eq / ts_val, 2)
                balance_latest["bvps"] = rt["bvps"]
                print(f"  [回退] bvps 从 total_equity/total_shares 推算: {rt['bvps']}")

        # 公司名称回退链
        # 1) 已从 push2 或 sina fallback 获取
        # 2) 从财报 raw_records 的 SECURITY_NAME_ABBR 提取
        if not rt.get("name") or rt["name"] == "":
            income_recs = result.get("income_data", {}).get("raw_records", [])
            balance_recs = result.get("balance_data", {}).get("raw_records", [])
            name_from_records = _extract_name_from_records(income_recs, balance_recs)
            if name_from_records:
                rt["name"] = name_from_records
                print(f"  [回退] name 从财报记录提取: {rt['name']}")
        # 确保 meta.name 同步
        result["meta"]["name"] = rt.get("name", "") or result["meta"].get("name", "")
        result["meta"]["industry"] = rt.get("industry", "") or result["meta"].get("industry", "")
        # 3. WACC 输入
        print("\n[3/7] 获取WACC输入数据...")
...
def main():
    ...
    # collaborative 模式下的目录处理
    if args.mode == 'collaborative' and args.data_dir:
        output_dir = os.path.join(args.data_dir, 'valuation-reports')
    ...
    # collaborative 模式下尝试读取已有 stock_info
    existing_rt = None
    if args.mode == 'collaborative' and args.data_dir:
        stock_info_path = os.path.join(args.data_dir, 'market-data', 'stock_info.json')
        if os.path.isfile(stock_info_path):
            try:
                with open(stock_info_path, 'r', encoding='utf-8') as f:
                    existing_rt = json.load(f)
                print(f"  [collaborative] 从 {stock_info_path} 读取已有行情数据")
            except Exception as e:
                print(f"  [collaborative] 读取 stock_info.json 失败: {e}，将重新获取")

    data = collector.collect_all()

    # 如果有已有行情数据，用它替换实时行情（避免重复请求）
    if existing_rt:
        data["real_time"] = existing_rt
