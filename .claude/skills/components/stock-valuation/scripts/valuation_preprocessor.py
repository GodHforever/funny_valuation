...
class DataCompletenessChecker:
    ...
    def _check_reverse(self):
        ...
        has_rt = (rt.get("current_price") or rt.get("price")) is not None
        has_wacc = wacc.get("status") == "success"
...
class EPSForecaster:
    ...
    def _generate_forecasts(self, trend_type, ttm_eps, vals, cagr_val, industry):
        ...
        if trend_type == "stable_growth" and cagr_val is not None:
            ...
            is_sustainable = any(kw.lower() in (industry or "").lower() for kw in SUSTAINABLE_KEYWORDS)
            if is_sustainable:
                # 可持续型（消费/医药）
...
class WACCCalculator:
    ...
    def calculate(self):
        ...
        # 资本结构（兼容新旧字段名）
        equity = safe_float(wacc_inputs.get("equity_market_cap") or wacc_inputs.get("total_market_cap"), default=0.0)
        if equity == 0.0:
            # 尝试从 real_time 获取
            rt = self.data.get("real_time", {})
            equity = safe_float(rt.get("total_market_cap") or rt.get("market_cap"), default=0.0)
...
class ValuationPreprocessor:
    ...
    def _build_real_time_snapshot(self):
        ...
        # 兼容新旧字段名
        price = rt.get("current_price") or rt.get("price")
        total_market_cap = safe_float(rt.get("total_market_cap") or rt.get("market_cap"), default=0.0)
        total_shares = bal.get("total_shares") or rt.get("total_shares")
        pb = rt.get("pb_mrq") or rt.get("pb")
        eps = rt.get("eps_ttm") or rt.get("eps")
        bvps_val = rt.get("bvps")
    ...
    def _ensure_critical_fields(self):
        """确保关键字段有值，缺失时从其他可用数据推算。"""
        rt = self.data.get("real_time") or self.data.get("stock_info") or {}

        # eps_ttm 回退
        if not rt.get("eps_ttm") and not rt.get("eps"):
            income = self.data.get("income_data", {}).get("annual_series", {})
            eps_series = income.get("eps", [])
            if eps_series:
                latest = sorted([e for e in eps_series if e.get("value") is not None],
                                key=lambda x: x.get("year", 0), reverse=True)
                if latest:
                    rt["eps_ttm"] = latest[0]["value"]

        # bvps 回退
        if not rt.get("bvps"):
            balance = self.data.get("balance_data", {})
            latest_bal = balance.get("latest", {})
            if latest_bal.get("bvps"):
                rt["bvps"] = latest_bal["bvps"]
            elif latest_bal.get("total_equity") and rt.get("total_shares"):
                shares = safe_float(rt["total_shares"])
                if shares > 0:
                    rt["bvps"] = round(safe_float(latest_bal["total_equity"]) / shares, 4)

        # total_shares 回退
        price = safe_float(rt.get("current_price") or rt.get("price"))
        mcap = safe_float(rt.get("total_market_cap") or rt.get("market_cap"))
        if not rt.get("total_shares") and mcap and price and price > 0:
            rt["total_shares"] = round(mcap / price, 4)

        # total_market_cap 回退
        if not mcap and price and rt.get("total_shares"):
            rt["total_market_cap"] = round(price * safe_float(rt["total_shares"]), 4)
